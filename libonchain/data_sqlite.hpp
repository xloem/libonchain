...
public:
    ...
    DataSqlite(std::string const & filename = "libonchain.sqlite", std::string const & table = "libonchain", std::string const & keys = "key", std::vector<std::string> const & values = {"value"});
    ~DataSqlite();

    virtual void add(std::string const & key, std::vector<std::vector<uint8_t>> const & values) override;
